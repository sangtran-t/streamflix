...
interface NavProps {
  /**
   * string  → logged in, show avatar with this initial
   * null    → auth not yet resolved, hide right column (no flash)
   * undefined → logged out, show "Sign in" link
   */
...
/**
 * Floating island nav — a distinct glass block that hovers above the page.
 * Always visible (not transparent-at-top); gains elevation on scroll.
 * Hidden entirely on /watch/* (player screen).
 */
export function Nav({ userInitial }: NavProps) {
  ...
  // indicator is already at the correct position on first render — no visible
  // "slide from left" on hard-refresh.  Also re-measures after fonts load,
  // since Bricolage Grotesque can shift button widths when it replaces the
  // fallback font.
  useLayoutEffect(() => {
    const measure = () => {
    ...
    measure();
    // Re-measure after font swap — document.fonts.ready resolves once all
    // @font-face fonts have finished loading.
  ...
  return (
    <nav
      style={{
        /* Floating block — detached from all edges */
        position: 'fixed',
        top: 16,
        left: '50%',
        ...
        /* Three-column grid: logo | tabs | avatar */
        display: 'grid',
    ...
    >
      ...
      >
        ...
        <span
          ...
          style={{
            ...
            opacity: ind.on ? 1 : 0,
            // No position transition until the first measurement is applied —
            // prevents the pill from sliding in from left:0 on hard refresh.
      ...
      {/* ── Avatar / sign-in ──────────────────────────── */}
      <div style={{ justifySelf: 'end', minWidth: 72 }}>
        {/* null = auth pending → render nothing to avoid sign-in flash */}
        {userInitial !== null && (userInitial ? (
